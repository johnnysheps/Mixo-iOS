...
func getUserData(_ response:Dictionary<String,String>, _ userUID:String) {
...
struct ProfilePic: View {
    ...
    init(response:Dictionary<String, String>, screenWidth:CGFloat, avatarQuadrant:CGFloat, midX:CGFloat, midY:CGFloat) {
...
struct Row: View {
            ...
            HStack(alignment:.top, spacing:0) {
                ...
                VStack(alignment:.leading, spacing:0) {
                    GeometryReader { geo in
                        ...
                        ProfilePic.init(response: self.response, screenWidth: self.screenWidth, avatarQuadrant: self.avatarQuadrant, midX: geo.frame(in: .local).midX, midY: geo.frame(in: .local).midY)
                            .cornerRadius(50)
                    } // Geometry Reader
    ...
    init(_ response:Dictionary<String, String>, _ age:Int) {
...
struct DiscoveryVC: View {
            ...
            ScrollView {
                LazyHGrid(rows: rows, alignment: .center, spacing:0, pinnedViews: []) {
                    ForEach((0...persons.count-1), id:\.self) {
                        Row.init(self.response, persons[$0].age)
                    }
    ...
    init() {
        ...
        // print(userUID);
        self.response = ["initializedAt":"DiscoveryVC"];
        print(getUserData(self.response, userUID));
